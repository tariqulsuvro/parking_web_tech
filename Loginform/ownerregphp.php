<?php
    ...
    session_start();
    $uName = $uPass = $uMail = $uMobile = $uNid = $uAdd = $u_p_area = $u_p_division =  $uSpc = $err = $err1 = $err2 = $err3 = $err4 = $err5 = $msg = $uNameInDB = $uMailInDB = $uMobileInDB = $uNidInDB = $uAddInDB = "" ;
    ...
    if(!empty($_POST['addr'])){
        $uAdd = mysqli_real_escape_string($conn, $_POST['addr']);
      }

    if(!empty($_POST['p_area'])){
      $u_p_area = mysqli_real_escape_string($conn, $_POST['p_area']);
    }

    if(!empty($_POST['p_division'])){
      $u_p_division = mysqli_real_escape_string($conn, $_POST['p_division']);
    }

    if(!empty($_POST['spc'])){
        $uSpc = mysqli_real_escape_string($conn, $_POST['spc']);
      }
    ...
    $sqlUserCheck = "SELECT id, username, email, mobile_no, nid FROM owner WHERE username='$uName' OR email='$uMail' OR mobile_no='$uMobile' OR nid='$uNid' ";
    ...
    if($n==0){
      $sql = "INSERT INTO owner (username, password, email, mobile_no, nid, address, parking_area, division, space)
              VALUES ('$uName','$uPassToDB','$uMail','$uMobile','$uNid','$uAdd','$u_p_area','$u_p_division','$uSpc');";
